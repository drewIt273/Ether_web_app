...
export class Registry<V extends any> {
    reg: Record<string, V>
    constructor(src: Record<string, V> | {} = {}) {
        let o: Record<string, any> = {}
        this.reg = strictObject(src) ? {...src} : o
    }
     ...
     */
    write(data: V, key = ''): string {
        if (typeof key !== 'string' || !key) key = this.#generatekey()
        this.reg[key] = data
        return key
    }
     ...
     */
    forEach(callback: (value: V, key: string) => any) {
     ...
     */
    filter(predicate: (v: V, k?: string) => boolean) {
     ...
     * 
     */
    mutate(predicate: (v: V, k?: string) => boolean) {
        for (const [k, v] of Object.entries(this.reg)) {
            if (predicate(v, k)) this.reg[k] = v
        }
        return this
    }
     ...
     */
    merge(...others: [Registry<any>, string][]) {
        others.forEach(other => {
            const o = other[0], r = other[1], k = this.includesKey(r) ? `_${r}` : r
            Object.defineProperty(this.reg, k, o.reg)
        })
    }
     ...
     */
    parse(src: string): Registry<any> | undefined {
